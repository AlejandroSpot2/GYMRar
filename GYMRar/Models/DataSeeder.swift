...
enum DataSeeder {
    static func seedIfNeeded(container: ModelContainer) {
        ...
        // Check exercises independently - seed if missing
        let existingExercises = try? ctx.fetch(FetchDescriptor<Exercise>())
        let needsExercises = (existingExercises ?? []).isEmpty

        // Check gyms for initial setup (gyms, calibrations, routine)
        let existingGyms = try? ctx.fetch(FetchDescriptor<Gym>())
        let needsInitialSetup = (existingGyms ?? []).isEmpty

        // Exercise catalog - always seed if missing
        if needsExercises {
        ...
        // Initial setup - only on first run (no gyms exist)
        ...
        if needsExercises || needsInitialSetup {
            try? ctx.save()
        }
    }
}
